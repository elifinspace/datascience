...
import datascience.visuals as vs
...
col_list = list(features_final)
import numpy as np

corr_ = np.empty((len(col_list), (len(col_list))))
count = 0
for col in col_list:
    col_list.remove(col)
    for col_2 in col_list:
        if features_final[col].corr(features_final[col_2]) > 0.8:
            print(col, col_2)
            count += 1
